...
if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert({
            projectId: process.env.FIREBASE_PROJECT_ID || "nijahomzs",
...
async function processPropertiesBatch(lastDoc = null) {
    try {
        ...
        let propertyQuery = db.collection('properties').limit(100); // Smaller batches for stability
        if (lastDoc) {
        ...
        snapshot.forEach(docSnap => {
            ...
            if (data.type === 'property') {
                updates.type = admin.firestore.FieldValue.delete();
                needsUpdate = true;
            }
