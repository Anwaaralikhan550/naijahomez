...
export async function GET(request) {
  try {
    ...
    const validateClaimToken =
      onboardingModule.validateClaimToken ||
      onboardingModule.default?.validateClaimToken;

    const result = await validateClaimToken({ rawToken: token });
    const status = result.valid ? 200 : 400;

    if (result.valid) {
      logEvent({
        claimTokenId: result.tokenId,
